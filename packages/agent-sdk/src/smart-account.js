/**
 * aegis-agent-sdk — smartAccount official recommended entry (facade)
 *
 * PURPOSE
 * ───────
 * Sprint 2.2: this is the SDK-level "official entry point" for the Smart
 * Account EVM path. An agent that only depends on aegis-agent-sdk can
 * reach the full on-chain hard-policy layer without importing chain-eth
 * directly:
 *
 *     import { smartAccount } from 'aegis-agent-sdk';
 ...
 * WHY LAZY
 * ────────
 * aegis-chain-eth statically imports THIS package (canonical/verifier
...
async function loadChainEth() {
  if (!_modulePromise) {
    _modulePromise = import('aegis-chain-eth').then((m) => ({
      ...
      if (err && /cannot find package|MODULE_NOT_FOUND/i.test(String(err.message))) {
        throw new Error(
          'smartAccount helpers require aegis-chain-eth, which is an optional peer ' +
          'dependency of aegis-agent-sdk. Install it explicitly: ' +
 ...
 * @see aegis-chain-eth createSmartAccountClient
 */
 ...
 * @see aegis-chain-eth signSmartAccountIntent
 */
 ...
 * @see aegis-chain-eth verifySmartAccountIntent
 */
 ...
 * @see aegis-chain-eth hashIntentDigest
 */
 ...
 * @see aegis-chain-eth deploySmartAccount
 */
 ...
 * @see aegis-chain-eth createChainConnection
 */
 ...
 * @see aegis-chain-eth createChainProvider
 */
 ...
 * @see aegis-chain-eth intentToStruct
 */
 ...
 * @see aegis-chain-eth payloadDigest
 */
 ...
 * @see aegis-chain-eth decodeRevert
 */
