...
> 项目：aegis-vault 上层组合代码安全审计
> 版本：1.0（2026-09-02）
> 状态：可发给审计方
> 依据：`Aegis Vault-pivot-roadmap` Phase 2（关键路径）

---

## 1. 背景与目标

`aegis-vault` 是 agent 密钥托管/授权库：PQC（Dilithium2）根身份 → 分层密钥派生 → 限额/接管/会话收窄。底层依赖 `@noble/post-quantum` 已有 Noble 团队审计覆盖，**不在本次范围内**。
