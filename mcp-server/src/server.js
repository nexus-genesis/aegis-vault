/**
 * Aegis Vault MCP Server — core (tool definitions + handlers + Server)
 *
...
async function bootChainEnv() {
  ...
  const { createChainProvider } = await import('aegis-chain-eth');
  const { loadSmartAccountArtifact } = await import('aegis-chain-eth/test-helpers/load-artifact');
  ...
  if (cfg.rpcUrl) {
    ...
    try {
      ({ createLocalChain } = await import('aegis-chain-eth/test-helpers/local-chain'));
    } catch (impErr) {
...
async function restoreSmartAccounts({ provider, abi, cfg }) {
  ...
  const { createChainConnection } = await import('aegis-chain-eth');
...
async function handleGenerateKeyPair() {
  const { generateKeyPair: gk, secureZero } = await import('aegis-vault');
  const { publicKey, privateKey } = await gk();
  let chainAddresses;
  try {
    const { deriveChainAddresses } = await import('aegis-chain-adapters');
...
async function handleSmartAccountSetup(args) {
  ...
  if (existingEntry) {
    ...
    const { deploySmartAccount } = await import('aegis-chain-eth');
...
const TOOLS = [
  ...
  {
    name: 'get_status',
    description: 'Get current Aegis Vault network status (block height, agents, network age).',
    inputSchema: { type: 'object', properties: {} },
  },
  {
    name: 'register_agent',
    description: 'Register a new AI Agent on the Aegis Vault network with a real Dilithium2 key and Proof-of-Work. Returns the on-chain address and custody token.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  { name: 'get_agents', description: 'List all AI Agents registered on Aegis Vault.', inputSchema: { type: 'object', properties: {} } },
  {
    name: 'get_agent',
  ...
  },
  {
    ...
    description: 'Validate whether a string is a well-formed Aegis Vault ng1 address.',
...
export function createServer() {
  ...
  const server = new Server(
    { name: 'aegis-agent-mcp', version: process.env.MCP_VERSION || '0.3.0' },
    { capabilities: { tools: {} } },
  );
